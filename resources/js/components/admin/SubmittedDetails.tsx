...
export function SubmittedDetails({ provider }: { provider: OwnedProvider }) {
  ...
  return (
    <section className="card p-5">
      <h2 className="text-lg font-bold text-ink-900">Submitted details</h2>

      <dl className="mt-3 grid gap-3 text-sm text-ink-700 sm:grid-cols-2">
        {rows.map(([label, value]) => (
          <div key={label}>
            <dt className="text-xs font-bold tracking-wide text-ink-400 uppercase">{label}</dt>
      ...
      {provider.description ? (
        <>
          <h3 className="mt-5 text-sm font-bold text-ink-900">Description</h3>
      ...
      <h3 className="mt-5 text-sm font-bold text-ink-900">Services</h3>
