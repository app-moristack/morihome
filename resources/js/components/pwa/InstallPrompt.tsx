        ...
        <div className="min-w-0 flex-1">
          ...
          {canPromptNatively ? (
            <p className="mt-0.5 text-xs leading-relaxed text-ink-500">
              {t('Open it like an app, full screen and one tap away.')}
            </p>
          ) : (
            <p className="mt-0.5 flex flex-wrap items-center gap-1 text-xs leading-relaxed text-ink-500">
              {t('Tap')}
              <Share className="inline size-3.5 text-ink-700" aria-label={t('the Share button')} />
              {t('then')}
              <SquarePlus className="inline size-3.5 text-ink-700" aria-hidden />
              <span className="font-semibold">{t('Add to Home Screen')}</span>.
            </p>
          )}
          ...
          <div className="mt-3 flex items-center gap-2">
            {canPromptNatively ? (
              ...
              >
                {t('Show me how (')}
                {platform === 'ios' ? 'iPhone' : t('my phone')})
              </Link>
            )}
          </div>
        </div>
