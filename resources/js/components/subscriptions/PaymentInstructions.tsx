    ...
    return (
      <div className="rounded-xl border border-success/30 bg-green-50 p-4 text-sm text-ink-700">
        <strong className="flex items-center gap-2 text-ink-900">
          <CircleCheck className="size-5 text-success" aria-hidden /> No payment required
        </strong>
        <p className="mt-1">Your free subscription request will await admin approval.</p>
      </div>
    )
  }

  return (
    <section className="rounded-xl border border-brand-300 bg-brand-50 p-4" aria-label="Payment procedure">
      <h3 className="font-bold text-ink-900">
        {isAfterRequest ? 'Complete payment to activate your request' : 'Payment after account creation'}
      </h3>
      <p className="mt-1 text-sm text-ink-600">
        Total: <strong className="text-ink-900">Rs {total.toLocaleString('en-MU')}</strong>
      </p>
      <ol className="mt-3 grid gap-3 text-sm text-ink-700">
        <li className="flex gap-3">
          ...
          <span>
            Send the full amount by <strong>Juice</strong> to{' '}
        ...
        <li className="flex gap-3">
          ...
          <span>
            Send your proof of payment on WhatsApp to{' '}
            <a
      ...
      <p className="mt-3 text-xs font-medium text-ink-500">
        An admin will verify the payment, set the start and end dates, and activate the subscription.
      </p>
    </section>
  )
}
